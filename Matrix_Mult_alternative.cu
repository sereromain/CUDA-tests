...
---------------  Evaluation de calcul GPU  ---------------

Ce code permet la multiplication de 2 matrices pouvant être non-carrées.
De plus il est meilleur que la multiplication de matrice basique en CUDA (celle qui consiste à charger dans chaque Thread la ligne et colonne afin de les multipliées)
Car à la place, ce code va charger dans la mémoire __Shared__ du Block des morceaux des 2 matrices afin de cacluler la multiplication en plusieurs itérations 
Mais aussi en partageant la mémoire entre les Threads d'un même Block.

Pour exécuter le code, il suffit de taper la commande : 
!code=Matrix_Mult_SERE_ROMAIN && nvcc -arch=sm_35 -o $code.o $code.cu && ./$code.o
